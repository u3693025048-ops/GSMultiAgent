...
class RAGRetrievalTool(RAGToolMixin):
    ...
    async def execute(self, query: str, top_k: int = 5) -> Dict[str, Any]:
        ...
        try:
            # Restrict to knowledge_base documents only.
            # kb_only=True (default) applies a Python-side strict post-filter so
            # PE JSON files and any other untagged docs are always excluded,
            # regardless of Chroma metadata-filter support.
